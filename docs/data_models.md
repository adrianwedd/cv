...
> **Note**: Earlier versions emitted richer `professional_metrics` (scores, raw_metrics, growth_indicators) and `skill_analysis` blocks. The current committed `activity-summary.json` is trimmed to the keys shown above; those legacy blocks are no longer emitted.
...
### 5. Integrity & Verification Files
...
*   `verification-summary.json` — Summary output of the content/claim verification run.
*   `claim-verification-report.json` — Detailed per-claim verification results.
*   `placeholder-replacement-validation.json` — Validation report for placeholder-metric replacement during generation.
*   `github-activity.json` — Raw GitHub activity data collected during the pipeline.
*   `content-audit.json` — Content audit output (gitignored).
