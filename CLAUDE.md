...
The spine's `evidence_hierarchy` block ranks sources when they conflict: primary artifact/record → explicit user-confirmed fact → corroborated spine claim → QA'd application interpretation → legacy CV/LinkedIn prose. A QA'd application is an audited *interpretation*, never primary evidence — VERIFIED status requires rank 1 or 2 somewhere in the evidence chain; rank-4-only evidence supports at most QUALIFIED.
